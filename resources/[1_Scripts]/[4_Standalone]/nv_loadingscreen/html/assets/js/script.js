...
if (showStaffTeam){
	...
	$(".panel.staffteam .title").text("Fondateurs") // 👈 Ajoute cette ligne
...
$(document).ready(function() {
    // Code pour les flèches et la musique
    ...
    // Créer les flèches de navigation si showMusicSelector est true
    if (showMusicSelector) {
        $(".music-selector").append(`
            <button id="prev-music" class="music-button">←</button>
            <button id="next-music" class="music-button">→</button>
        `);
...
$(document).ready(function() {
    ...
    if ($("#musicChangeBtn").length > 0) {
        ...
        $("#musicChangeBtn").on("click", function() {
            ...
            // Si l'élément audio existe, changer sa source et le jouer
            if ($("#audioPlayer").length > 0) {
                const audioPlayer = $("#audioPlayer")[0];
                audioPlayer.src = musicList[currentMusicIndex];
                audioPlayer.play();
                
                // Si le son était en pause, il faut mettre à jour l'état
                if (isPaused) {
                    isPaused = false;
                    $(".mini-buttons button:nth-child(2)").removeClass("act");
                }
                
                // Si le son était en mute, il faut s'assurer qu'il soit toujours en mute
                if (isMute) {
