...
function main_vehicles_spawn_showContentThisFrame(playerGroup)
	...
	for _k, vehicle in pairs(_var.vehicle.list) do
		_var.menu.findedVehicle = string.find(vehicle.label, tostring((_var.menu.searchVehicle):gsub("^%l", string.upper)))
		if _var.menu.findedVehicle then
			RageUI.ButtonWithStyle(vehicle.label .. " ~c~[" .. string.upper(vehicle.type) .. "]", _U("main_vehicles_spawn_spawn_desc", vehicle.label, vehicle.type), { RightBadge = RageUI.BadgeStyle.Car }, true, function(_h, _a, Selected)
				if Selected then
