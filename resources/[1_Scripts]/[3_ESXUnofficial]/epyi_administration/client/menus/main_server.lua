...
function main_server_showContentThisFrame(playerGroup)
	if GetResourceState("cd_easytime") == "started" then
		RageUI.List(_U("main_server_weather"), _var.menu.weatherArray, _var.menu.weatherArrayIndex, _U("main_server_weather_desc"), {}, Config.Groups[playerGroup].Access["submenu_server_weather"] and not _var.menus.admin.cooldowns.items, function(_h, _a, Selected, Index)
		...
		RageUI.List(_U("main_server_time"), _var.menu.timeArray, _var.menu.timeArrayIndex, _U("main_server_time_desc"), {}, Config.Groups[playerGroup].Access["submenu_server_time"] and not _var.menus.admin.cooldowns.items, function(_h, _a, Selected, Index)
		...
		RageUI.Checkbox(_U("main_server_blackout"), _U("main_server_blackout_desc"), _var.menu.blackoutCheckbox, {
			...
			TriggerServerEvent("cd_easytime:ForceUpdate", {
				blackout = _var.menu.blackoutCheckbox,
			})
		end, function()
			if not Config.Groups[playerGroup].Access["submenu_server_blackout"] then
				return
	...
	RageUI.ButtonWithStyle(_U("main_server_clearall"), _U("main_server_clearall_desc"), { RightLabel = "→" }, Config.Groups[playerGroup].Access["submenu_server_clearall"] and not _var.menus.admin.cooldowns.items, function(_h, _a, Selected)
		if Selected then
	...
	RageUI.ButtonWithStyle(_U("main_server_clearvehicle"), _U("main_server_clearvehicle_desc"), { RightLabel = "→" }, Config.Groups[playerGroup].Access["submenu_server_clearvehicles"] and not _var.menus.admin.cooldowns.items, function(_h, _a, Selected)
		if Selected then
			Citizen.CreateThread(function()
				_var.menus.admin.cooldowns.items = true
				local ped = PlayerPedId()
	...
	RageUI.ButtonWithStyle(_U("main_server_clearpeds"), _U("main_server_clearpeds_desc"), { RightLabel = "→" }, Config.Groups[playerGroup].Access["submenu_server_clearpeds"] and not _var.menus.admin.cooldowns.items, function(_h, _a, Selected)
		if Selected then
			Citizen.CreateThread(function()
				_var.menus.admin.cooldowns.items = true
				local ped = PlayerPedId()
