...
function main_vehicles_current_custom_showContentThisFrame(playerGroup)
	...
	RageUI.ButtonWithStyle(_U("main_vehicles_current_fullperf"), _U("main_vehicles_current_fullperf_desc"), {}, Config.Groups[playerGroup].Access["submenu_vehicles_current_custom_fullperf"], function(_h, _a, Selected)
		if Selected then
	...
	RageUI.ButtonWithStyle(_U("main_vehicles_current_plate"), _U("main_vehicles_current_plate_desc"), { RightLabel = "→" }, Config.Groups[playerGroup].Access["submenu_vehicles_current_custom_plate"], function(_h, _a, Selected)
		if Selected then
			if not pedVehicle or GetPedInVehicleSeat(pedVehicle, -1) ~= ped then
				ESX.ShowNotification(_U("self_not_in_vehicle"))
				return
	...
	RageUI.List(_U("main_vehicles_current_color_main"), _var.vehicle.paintColorsArray, _var.vehicle.paintColorsArrayIndexMain, _U("main_vehicles_current_color_main_desc"), {}, Config.Groups[playerGroup].Access["submenu_vehicles_current_custom_color_main"], function(_h, Active, _s, Index)
		...
		if Active then
			...
			SetVehicleColours(pedVehicle, _var.vehicle.paintColors[_var.vehicle.paintColorsArrayIndexMain][2], colorSecondary)
		end
		if not Active then
			if not pedVehicle or GetPedInVehicleSeat(pedVehicle, -1) ~= ped then
				return
	...
	RageUI.List(_U("main_vehicles_current_modtype"), _var.vehicle.mods, _var.menu.modArrayIndex, _U("main_vehicles_current_modtype_desc"), {}, Config.Groups[playerGroup].Access["submenu_vehicles_current_custom_mods"], function(_h, Active, _s, Index)
	...
	RageUI.List(_U("main_vehicles_current_modvariant"), mods[_var.menu.modArrayIndex].list, _var.menu.modVariantArrayIndex, _U("main_vehicles_current_modvariant_desc"), {}, Config.Groups[playerGroup].Access["submenu_vehicles_current_custom_mods"], function(_h, Active, _s, Index)
